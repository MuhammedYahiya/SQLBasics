...
SELECT Winner, SUM(Winner_Pts) AS total_point FROM superbowls GROUP BY Winner;

SELECT * FROM superbowls WHERE Loser LIKE "%Francisco%" 

SELECT Winner, SUM(Winner_Pts) AS total_point FROM superbowls GROUP BY Winner HAVING total_point >150;
...
SELECT Winner,Winner_Pts,
    CASE 
        WHEN Winner_Pts > 39 THEN "Hight Winning point"
        WHEN Winner_Pts >  20 THEN "Medium Winning point"
        ELSE "Low winning point"
    END "Winnig Point cate"
FROM superbowls;


SELECT * FROM superbowls WHERE Date IN("Feb 2 2020","Feb 3 2019","Feb 4 2018","Feb 5 2017","Feb 7 2016");
